...
const statusClass = {
  ...
  milestone: "bg-gold/20 text-navy ring-gold/35"
};
    ...
    <section className="rounded-[1.5rem] bg-white p-4 shadow-card">
      ...
      <div className="mt-4 grid grid-cols-[repeat(15,minmax(0,1fr))] gap-1 sm:grid-cols-[repeat(25,minmax(0,1fr))]">
        {days.map((day) => {
          ...
          const visualStatus = isMilestone && day.status !== "completed" && day.status !== "available" ? "milestone" : day.status;
          return (
            <button
              key={day.dayNumber}
              type="button"
